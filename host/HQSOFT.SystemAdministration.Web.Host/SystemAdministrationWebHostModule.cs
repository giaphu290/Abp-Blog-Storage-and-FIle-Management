...
using HQSOFT.SystemAdministration.Menus;
using HQSOFT.SystemAdministration.MultiTenancy;
using HQSOFT.SystemAdministration.Web;
...
[DependsOn(
    typeof(SystemAdministrationWebModule),
...
public class SystemAdministrationWebHostModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(SystemAdministrationWebHostModule).Assembly
            );
        });
    }
    ...
    private void ConfigureMenus(IConfiguration configuration)
    {
        ...
        {
            options.MenuContributors.Add(new SystemAdministrationWebHostMenuContributor(configuration));
        });
        
        Configure<AbpToolbarOptions>(options =>
        {
            options.Contributors.Add(new SystemAdministrationToolbarContributor());
        });
    }
    ...
    private void ConfigureAuthentication(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAuthentication(options =>
            ...
            .AddAbpOpenIdConnect("oidc", options =>
            {
                ...
                options.Scope.Add("SystemAdministration");
            });
    }
    ...
    private void ConfigureAutoMapper()
    {
        ...
        {
            options.AddMaps<SystemAdministrationWebHostModule>();
        });
    }
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<SystemAdministrationWebModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}..{0}src{0}HQSOFT.SystemAdministration.Web", Path.DirectorySeparatorChar)));
            });
        }
    }
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("SystemAdministration");
        if (!hostingEnvironment.IsDevelopment())
        {
