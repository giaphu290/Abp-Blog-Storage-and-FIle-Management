...
function initializeDataChangeHandling() {
    // Thêm sự kiện lắng nghe cho các thao tác có thể làm thay đổi dữ liệu
    const inputs = document.querySelectorAll('input, textarea, select');

    inputs.forEach(input => {
        input.addEventListener('change', MarkDataChanged);
    });

    window.addEventListener("beforeunload", function (event) {
        if (isChanged) {
            event.preventDefault();
            event.returnValue = ''; // Hiển thị cảnh báo tiêu chuẩn khi tải lại hoặc đóng trang
        }
    });
}
...
// REPORT DESIGNER
function registerReportDesignerUtilsBlazorMethod(blazorObject) {
    window.blazorObject = blazorObject;
}

function invokeReportDesignerUtilsBlazorPopup() {
    if (window.blazorObject) {
        window.blazorObject.invokeMethodAsync('ResetReportToolbarItemsAsync');
    }
}

function onReportDesignerLoad() {
    customizeReportDesigner();
}

function customizeReportDesigner() {
    var iframe = document.getElementById('report-designer');
    iframe.onload = function () {
        ...
        invokeReportDesignerUtilsBlazorPopup();
    };
}
