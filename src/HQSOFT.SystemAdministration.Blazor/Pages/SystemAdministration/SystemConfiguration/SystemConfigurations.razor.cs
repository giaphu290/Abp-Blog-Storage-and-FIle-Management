...
namespace HQSOFT.SystemAdministration.Blazor.Pages.SystemAdministration.SystemConfiguration
{
    public partial class SystemConfigurations
    {
        ...
        private SystemConfigurationUpdateDto EditingDoc { get; set; }
        private List<SystemConfigurationDto> SystemConfigurationList { get; set; } = new List<SystemConfigurationDto>();
        ...
        /*===================================== Initialize Section ====================================*/
        #region
        public SystemConfigurations(IConfiguration configuration, IAuditingManager auditingManager)
        {
            EditingDoc = new SystemConfigurationUpdateDto
            {
        ...
        protected virtual ValueTask SetToolbarItemsAsync()
        {
            ...
            {
                NavigationManager.NavigateTo($"/SystemAdministration/SystemConfigurations");
            },
            IconName.Undo,
            Color.Light);
            ...
            Toolbar.AddButton(
                    ...
                    requiredPolicyName: EditingDocId != Guid.Empty
                        ? CoreBackendPermissions.SystemConfigurations.Edit
                        : CoreBackendPermissions.SystemConfigurations.Create,
        ...
        #region Load Data Source for ListView & SystemConfigurations
        ...
        private async Task LoadDataAsync(Guid classId)
        {
            ...
            {
                EditingDoc = ObjectMapper.Map<SystemConfigurationDto, SystemConfigurationUpdateDto>(await SystemConfigurationsAppService.GetAsync(classId));
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (IsDataEntryChanged)
                    {
                        if (EditingDocId == Guid.Empty)
                        {
                            var screen = await SystemConfigurationsAppService.CreateAsync(ObjectMapper.Map<SystemConfigurationUpdateDto, SystemConfigurationCreateDto>(EditingDoc));
                            EditingDoc = ObjectMapper.Map<SystemConfigurationDto, SystemConfigurationUpdateDto>(screen);
                        ...
                        {
                            await SystemConfigurationsAppService.UpdateAsync(EditingDocId, EditingDoc);
                            EditingDoc = ObjectMapper.Map<SystemConfigurationDto, SystemConfigurationUpdateDto>(await SystemConfigurationsAppService.GetAsync(EditingDocId));
                ...
                await InvokeAsync(async () =>
                {
                    IsDataEntryChanged = false;
                    await UpdateDataChangeStatus(false);
                    NavigationManager.NavigateTo($"/SystemAdministration/SystemConfigurations/{EditingDocId}");
                });
        ...
        private async Task DeleteAsync()
        {
            if (EditingDocId != Guid.Empty)
            {
                ...
                if (confirmed)
                {
                    try
                    {
                        await SystemConfigurationsAppService.DeleteAsync(EditingDocId);
                        await InvokeAsync(async () =>
                        {
                            NavigationManager.NavigateTo("/SystemAdministration/SystemConfigurations");
        ...
        private async Task<bool> HandleValidSubmit()
        {
            ...
            bool isDuplicateCode = (await SystemConfigurationsAppService.GetExistingDataByField(nameof(EditingDoc.Code), EditingDoc.Code, EditingDocId)).Any();
